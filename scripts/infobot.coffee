...
module.exports = (robot) ->
  ...
  robot.hear /(.+)(^|\s)(is|are)(\s|$)(.+)/i, (msg) =>
    [key, value] = [msg.match[1], msg.match[5]]
    factoid = @factoids.set key, value, msg.message.user.name

  # Learn a factoid by being addressed directly.
  # robot.respond /learn (.{3,}) = ([^@].+)/i, (msg) =>
  #   [key, value] = [msg.match[1], msg.match[2]]
  #   factoid = @factoids.set key, value, msg.message.user.name
  #
  #   if factoid.value?
  #     msg.send "Ok #{msg.message.user.name}, #{key} is now #{factoid.value}"
  ...
  robot.hear /\?+\s*$/i, (msg) =>
    ...
    msgtext = msgtext.replace(/^where is\s/i, '')
    msgtext = msgtext.replace(/\s+\?$/, '?')

    # Must match ^, else factoids with "whois" anywhere break.
    msgtext = msgtext.replace(/^whois\s/i, '')

    msgtext = msgtext.replace(/^who is\s/i, '')
    msgtext = msgtext.replace(/^what is (a|an)?/i, '')
    msgtext = msgtext.replace(/^how do i\s/i, '')
    msgtext = msgtext.replace(/^where can i (find|get|download)/i, '')
    msgtext = msgtext.replace(/^how about\s/i, '')
    msgtext = msgtext.replace(/\sda\s/ig, ' the ')

    # Clear the string of useless words.
    msgtext = msgtext.replace(/^(stupid )?q(uestion)?:\s/i, '')
    msgtext = msgtext.replace(/^(does )?(any|ne)(1|one|body) know\s/i, '')

    msgtext = msgtext.replace(/^[uh]+m[,\.]* +/i, '')

    msgtext = msgtext.replace(/^well([, ]+)/i, '')
    msgtext = msgtext.replace(/^still([, ]+)/i, '')
    msgtext = msgtext.replace(/^(gee|boy|golly|gosh)([, ]+)/i, '')
    msgtext = msgtext.replace(/^(well|and|but|or|yes)([, ]+)/i, '')

    msgtext = msgtext.replace(/^o+[hk]+(a+y+)?([,. ]+)/i, '')
    msgtext = msgtext.replace(/^g(eez|osh|olly)([,. ]+)/i, '')
  ...
  robot.hear new RegExp("^[#{prefix}]([\\w\\s-]{2,}\\w)( @.+)?", 'i'), (msg) =>
    ...
    to = msg.message.user.name #msg.match[2]
    if not fact? or fact.forgotten
      msg.send "#{msg.random DUNNO}, #{to.trim()}."
    else
      fact.popularity++
